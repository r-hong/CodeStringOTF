...
def dataGenerator(N,M):
	'''
	This is a small function to generate N randomly distrubuted gaussian clouds of data. Each cloud has M points.  
	'''
        def rnlist(N):
        #return a list of N random integers in the range 1-9    
                n0=0
                list_a=[]
                while (n0<N):
                        rn=int(np.random.random()*10)
                        if (rn in list_a):
                                a=0
                        else:
                                list_a.append(rn)
        ...
        for j in range(len(centerX)):
                for i in range(M):
                        x=random.gauss(centerX[j], 0.8)
                        y=random.gauss(centerY[j], 0.8)
                        z=random.gauss(centerZ[j], 0.8)
                        xx.append(x)
                        yy.append(y)
                        zz.append(z)
