...
def _make_parser():
	parser = argparse.ArgumentParser(description="This tool find the temporal order of the clusters in a dataset using the cumulative distributions functions (CDFs) of the points in each cluster. To do this task, it is assumed that the row index for each data point in the input file corresponds to a time stamp (i.e., as in a MD simulation). Resuming, from this calculation we obtain the reordering of the clusters in a sequence indicated by their time stamps and therefore the obtained ordering should be maintained when building local principal curves between subsequent pairs of clusters in time. In this tool we first evolve an initial guess formed by K points to find the K center of the clusters (using k-means). Next, the clusters CDFs are calculated and ploted. The temporal location of the CDFs in the final plot corresponds to the temporal order of the clusters.",epilog="Thanks for using clustersCDF.py!!")
